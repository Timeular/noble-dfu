...
var SecureDFU = exports.SecureDFU = function (_EventEmitter) {
  ...
  _createClass(SecureDFU, [{
    ...
    value: function update(device, init, firmware) {
      ...
      return this.connect(device).then(function () {
        ...
        _this2.log("complete, disconnecting...");
        return new Promise(function (resolve) {
          device.once("disconnect", function () {
            _this2.log("disconnect");
            resolve();
          });
        });
      });
    ...
    value: function transfer(buffer, type, selectType, createType) {
      ...
      return this.sendControl(selectType).then(function (response) {
        ...
        if (type === "init" && offset === buffer.byteLength && _this7.checkCrc(buffer, crc)) {
          _this7.log("init packet already available, skipping transfer");
          return;
        }

        _this7.progress = function (bytes) {
          this.emit("progress", {
            object: type,
        ...
        return _this7.transferObject(buffer, createType, maxSize, offset);
      });
    }
  }, {
    key: "transferObject",
    value: function transferObject(buffer, createType, maxSize, offset) {
      ...
      return this.sendControl(createType, view.buffer).then(function () {
        ...
        return _this8.transferData(data, start);
      }).then(function () {
        return _this8.sendControl(OPERATIONS.CALCULATE_CHECKSUM);
        ...
        if (_this8.checkCrc(data, crc)) {
          _this8.log("written " + transferred + " bytes");
          offset = transferred;
          return _this8.sendControl(OPERATIONS.EXECUTE);
        } else {
          _this8.log("object failed to validate");
        }
      }).then(function () {
        if (end < buffer.byteLength) {
          return _this8.transferObject(buffer, createType, maxSize, offset);
        } else {
          _this8.log("transfer complete");
        }
      });
    }
  }, {
    ...
    value: function transferData(data, offset, start) {
      ...
      return writeCharacterisitic(this.packetChar, buffer).then(function () {
        _this9.progress(offset + end);

        if (end < data.byteLength) {
          return _this9.transferData(data, offset, end);
        }
      });
    }
  }, {
...
function writeCharacterisitic(characteristic, buffer) {
