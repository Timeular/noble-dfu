...
function getFileName() {
  return new Promise(resolve => {
    ...
    rl.question(
      "Enter a URL or file path for the firmware package: ",
      answer => {
        rl.close();
        resolve(answer);
      }
...
function handleDeviceFound(peripheral, selectFn) {
  ...
  if (bluetoothDevices.length === 1) {
    // process.stdin.setRawMode(true);
    selectFn(bluetoothDevices[0].device)
  }
}
  ...
  getFileName()
    ...
    .then(zipFile => {
      try {
        ...
        throw new Error(
          "Unable to find manifest, is this a proper DFU dfuPackage?"
        );
      }
    })
    .then(content => {
      ...
      dfu.on("progress", event => {
        console.log((event.currentBytes / event.totalBytes).toFixed(2));
    ...
    .then(selectedDevice => {
      ...
      for (var type of ["softdevice", "bootloader", "softdevice_bootloader"]) {
        if (manifest[type]) {
          return updateFirmware(dfu, dfuPackage, manifest[type], device, type);
        }
      }
    })
    .then(device => {
